...
def signal_extract(facial_data,
    ...
    return output        
###############################################################################
def smooth_data(data, fs=1800, lowcut=10, highcut=100, order=1):
...
def moving_average(data, average_length):

    output = []
    for i in range (0, len(data)):
        if (i < (average_length/2)):  ## For starting elements
            average_range = range(0, average_length)
        elif (i + (average_length/2) > len(data)):
            average_range = range(-average_length, 0)  ## For end elements
        else:  ## Standard use within list
            average_range = range(-int(average_length/2), int(average_length/2)+1)
        
        sum_list = [data[i+j] for j in average_range]
        
        sum_point = 0
        sum_point += abs(sum_list[0])
        sum_point += abs(sum_list[1])
        sum_point += abs(sum_list[2])
        sum_point += abs(sum_list[3])
        sum_point += abs(sum_list[4])
        
        average_point = sum_point / average_length
        
        output.append(average_point)
...
plt.subplot(3, 1, 1)
plt.plot(f1[0:int(len(f1)/2)], 'k')

plt.subplot(3, 1, 2)
plt.plot(f2[0:int(len(f2)/2)], 'k')
     
plt.subplot(3, 1, 3)
plt.plot(f3[0:int(len(f3)/2)], 'k')
...
## FFT to determine HR/RR?
## Could also use interval between successive peaks for HR

smoothed_1 = smooth_data(s1)
smoothed_2 = smooth_data(s2)
smoothed_3 = smooth_data(s3)

x = [(i/fps) for i in range(0, len(smoothed_1))]

fig = plt.figure(figsize=(int(fig_width/2), fig_height)) ## Sets the size of the output plot, is set to extreme value to give finer detail in output image

plt.subplot(3, 1, 1)
plt.plot(x, smoothed_1, 'k')

plt.subplot(3, 1, 2)
plt.plot(x, smoothed_2, 'k')
     
plt.subplot(3, 1, 3)
plt.plot(x, smoothed_3, 'k')
    
smoothed_plot_name = "plot_ica_smoothed.png"
fig.savefig(smoothed_plot_name) ## Saves the output figure to a .png image file
print("\nSmoothed data figure saved to: " + smoothed_plot_name)
        
plt.close(fig)

red_wave, infra_wave = read_csv()

if ((max(f1) > max(f2)) & (max(f1) > max(f3))):
    rPPG = moving_average(smoothed_1, 5)
    print("Signal 1 chosen as rPPG")
elif ((max(f2) > max(f1)) & (max(f2) > max(f3))):
    rPPG = moving_average(smoothed_1, 5)
    print("Signal 2 chosen as rPPG")
elif ((max(f3) > max(f1)) & (max(f3) > max(f2))):
    rPPG = moving_average(smoothed_1, 5)
    print("Signal 3 chosen as rPPG")


x = [(i/fps) for i in range(0, len(rPPG))]
y = [(i/500) for i in range(0, (len(red_wave)-3000))]
...
plt.subplot(2, 1, 2)
plt.plot(y, red_wave[3000:len(red_wave)], 'k')

    
ppg_rppg_plot_name = "plot_ppg_rppg_comparison.png"
fig.savefig(ppg_rppg_plot_name) ## Saves the output figure to a .png image file
print("\nCompared data figure saved to: " + ppg_rppg_plot_name)
