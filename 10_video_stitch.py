...
video_files_directory = "E:\Dataset\\P001\\v1"
video_files = []
...
while((capA.isOpened()) & capB.isOpened() & capC.isOpened() & capD.isOpened()):
    ...
    if ((frames_passed % int(total_frames/100)) == 0):
                    print('Percent complete: %.2f' % (100 * frames_passed/total_frames),'%')
...
#        cv.imshow('Combined Sources', out_frame)  ## Uncomment to show output stitched frame
#        if cv.waitKey(1) & 0xFF == ord('q'):
#            break
    else:
        if frames_passed == 0:
            print ("Error occured open video files")
        break
